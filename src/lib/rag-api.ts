...
function getAuthToken(): string | null {
  ...
  for (const storeKey of ["hospitana-auth", "auth-storage", "auth", "user-storage"]) {
...
export async function askHospitana(question: string): Promise<ChatResponse> {
  ...
  const res = await fetch(`${API_ROOT}/api/v1/rag/chat`, {
    method: "POST",
    headers,
    body: JSON.stringify({
      question,
...
export async function resetHospitanaSession(): Promise<void> {
  const sessionId = getOrCreateSessionId();
  try {
    await fetch(`${API_ROOT}/api/v1/rag/reset`, {
      method: "POST",
